...
selection_price <- data%>% select(adm1,adm2,adm3,contains('_price_unit_ssp')) ### 22.12.2023 need to select the correct prices
...
mfs_afford_finance <- data %>% 
  mutate(afford_finance_true = if_else(rowSums(select(.,contains('affordability_financial.') 
                                                     & -contains('no_issues') 
                                                     & -contains('dont_know')
                                                     & -contains('pnta'))) > 0,1,0)) %>% #### 22.12.23 removing ptna from calculations 
  group_by(adm1,adm2,adm3) %>%
...
mfs_afford_price_vol <- data %>% 
  mutate(affordability_price_volatility = if_else(affordability_price_volatility == 'no',1,0)) %>% #### 22.12.23 option selection changed from "yes" to "no" 
  group_by(adm1,adm2,adm3) %>%
...
# input the number of categories selected below (note that these have to be within 1-3).
# aka food_supplier_local_single, food_supplier_imported_single, nfi_supplier_single
input_n_supplier_categories<- 3 # 12/22/23 count of categories for new denominator
...
mfs_resil_supply_diverse <- data %>% 
  ...
  ungroup() %>%
  mutate(food_supplier_local_single = case_when(food_supplier_local_single > 75 ~ 0,        # category 1
                                             food_supplier_local_single > 50 ~ 1,
                                             food_supplier_local_single > 25 ~ 2,
                                             food_supplier_local_single <= 25 ~ 3),
         food_supplier_imported_single = case_when(food_supplier_imported_single  > 75 ~ 0, # category 2
                                              food_supplier_imported_single  > 50 ~ 1,
                                              food_supplier_imported_single  > 25 ~ 2,
                                              food_supplier_imported_single  <= 25 ~ 3),
         nfi_supplier_single = case_when(nfi_supplier_single > 75 ~ 0,                      # category 3
...
mfs_infra_storage <- data %>% 
  mutate(infrastructure_storage = if_else(infrastructure_storage == 'no_store_facility_outside' |
                                            ...
                                            infrastructure_storage == 'other', 1,0)) %>% ### 22.12.23 infrastructure_storage == 'prefer_not_answer' removed from choices yielding score
  group_by(adm1, adm2, adm3) %>%
...
mfs <- mfs %>%
  ...
  mutate(mfs_accessibility_pillar_score = (rowSums(across(contains('access_score'))) /17) *25,
         ...
         mfs_resilience_pillar_score = (rowSums(across(contains('resil'))) /(length(stock_items)*3 + input_n_supplier_categories* 3 + 12)) *20, # 12/22/23 updated with new denominator
